# Copyright by Wolfgang Fuschlberger
#
...
# first Release: 2004-07-30. Modify by jerep6
#RELEASE="2008-04-26"
RELEASE="2013-01-06"
#
# The original version of the script is available at
...
#Assume that the users homedir are under /home
...
if [ "$DISTRO" = SUSE ]; then
  ...
  APPS="/bin/bash /bin/cp /usr/bin/dircolors /bin/ls /bin/mkdir /bin/mv /bin/rm /bin/rmdir /bin/sh /bin/su /usr/bin/groups /usr/bin/id /usr/bin/rsync /usr/bin/ssh /usr/bin/scp /sbin/unix_chkpwd /usr/bin/vim /usr/bin/touch /sbin/cryptsetup /bin/mount /usr/bin/clear /bin/chmod /bin/cat"
else
...
#Will be moved into the jail if user exists
HOMEDIR_OLD=$(eval echo ~$CHROOT_USERNAME)
...
chroot directory? The old users home directory ($HOMEDIR_OLD) will be move.
Are you REALLY sure?
...
#if (! grep -i "^$G" /etc/group > /dev/null 2>&1 ) ; then 
        groupadd -f $GROUP
#fi

# Define HomeDir for simple referencing
HOMEDIR="$JAILPATH/home/$CHROOT_USERNAME"
HOMEDIR_SYSTEM="/home/$CHROOT_USERNAME"
...
Moving files in $CHROOT_USERNAME's \$HOME to \"$HOMEDIR\"
"
usermod -s "$SHELL" -g $GROUP $CHROOT_USERNAME

mv $HOMEDIR_OLD $HOMEDIR
chmod 700 "$HOMEDIR"
ln -s $HOMEDIR $HOMEDIR_OLD
fi

if [ "$CREATEUSER" = "yes" ] ; then {
echo "Adding User \"$CHROOT_USERNAME\" to system"
ln -s $HOMEDIR $HOMEDIR_SYSTEM

useradd -m -N -g $GROUP -d "$HOMEDIR" -s "$SHELL" $CHROOT_USERNAME
chmod 700 "$HOMEDIR"
usermod -d "$HOMEDIR_SYSTEM" $CHROOT_USERNAME
...
cp /etc/inputrc ${JAILPATH}/etc/
cp -r /lib/terminfo ${JAILPATH}/lib/
